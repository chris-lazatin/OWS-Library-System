...
const statuses = ["Still Borrowed", "Returned", "For Replacement"];
const pageState = {
  search: "",
  filter: "",
...
function getFilteredRecords() {
  return getState().borrowRecords.filter((record) => {
    ...
    const matchesFilter = !pageState.filter || record.status === pageState.filter;
...
function renderBorrowRow(record) {
  ...
  return `
    <tr>
      ...
      <td>
        <select class="status-select" data-status-select data-record-id="${escapeHtml(record.id)}" aria-label="Update status for ${escapeHtml(record.borrowerName)}">
          ${statuses.map((status) => `<option value="${status}" ${record.status === status ? "selected" : ""}>${status}</option>`).join("")}
        </select>
      </td>
      <td>
